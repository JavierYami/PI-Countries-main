...
export const getCountryDetail = (id) => {
    return async function (dispatch) {
        await axios(`https://henrycountries.up.railway.app/countries/${id}`).then(response => dispatch({type:COUNTRY_DETAIL, payload:response.data})).catch(error =>  dispatch({type: ERROR, payload: error.message}))
    }
}
...
export const createActivity = (activity) => {
        return async function (dispatch) {
            let response = await axios.post('https://henrycountries.up.railway.app', activity).then(response => alert(`Activity ${activity.name} succesfully created`)).catch(error => alert(error.response.data.error));
...
export const getAllActivities = () => {
    return async function (dispatch) {
        let response = await axios ('https://henrycountries.up.railway.app/activities');
...
export const filterCountriesByActivity = (id) => {
    return async function (dispatch) {
        let response = await axios(`https://henrycountries.up.railway.app/countriesFilteredByActivity/${id}`);
...
export const updateInputValue = (inputValue) => {
    return async function (dispatch) {
        let response = await axios.get(`https://henrycountries.up.railway.app/countries/?name=${inputValue}`)
